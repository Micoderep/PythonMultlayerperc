...
def feedforward(layers,nodestruct,yarr,mlpwarr,trial,elmntsinmlp):
...
def trialruns(trialnum,inputnum,yarr,yinputs,elmntsinmlp,outputs,layers,nodestruct,mlpwarr):
    totalloss = 0
    for trial in range(trialnum): # loops through different inputs
        ...
        yarr[yarrposstart:yarrposstart+elmntsinmlp] = feedforward(layers,nodestruct,yarr,mlpwarr,trial,elmntsinmlp) # output corresponding to particular input.
...
totalloss, yarr = trialruns(trialnum,inputnum,yarr,yinputs,elmntsinmlp,outputs,layers,nodestruct,mlpwarr)
print(yarr)
print("Now weights")
print(mlpwarr)
...
# Here is where the layer loop is ***********************
for ifin in range(outputs): # For if you had multiple outputs.
    for nodej in range(int(nodestruct[layers-2])): # Loop for second to last layer!
        ...
        mlpwarr[presweight] = mlpwarr[presweight] - (lp*dsdw)/(trialnum)
        presweight += -1
        dsdw = 0
